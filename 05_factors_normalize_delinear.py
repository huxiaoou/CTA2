from setup_factor_and_portfolio import dt, pd
from setup_factor_and_portfolio import factors_exposure_dir, available_universe_dir
from setup_factor_and_portfolio import factors_exposure_norm_dir, factors_exposure_delinear_dir
...
from factors.XFuns import drop_df_rows_by_nan_prop, transform_dist, adjust_weight, sector_neutralize_factors_pool, normalize, delinear
import argparse
...
delinear_lib.initialize_table(t_table=database_structure[delinear_lib_id].m_tab, t_remove_existence=run_mode in ["O"])
...
for factor_lbl in selected_factors_pool:
    ...
    raw_factor_libs_manager[factor_lbl]["reader"].set_default(database_structure[factor_lbl].m_tab.m_table_name)
...
# --- core loop
for trade_date in trade_calendar.get_iter_list(t_bgn_date=bgn_date, t_stp_date=stp_date, t_ascending=True):
    # Load available universe
