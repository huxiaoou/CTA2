...
from config_factor import test_window_list, factors_pool_options, factors_return_lag_list
from custom.XFuns import fun_for_normalize_delinear
from custom.XFuns import fun_for_factors_return
import subprocess as sp

fix_bgn_date, md_stp_date = sys.argv[1], sys.argv[2]  # format = "YYYYMMDD", we suggest set fix_bgn_date = "20120101"
sep = "-" * 120 + "\n"

sp.run(["python", "00_A_instrument_return.py"])
print(sep)

sp.run(["python", "00_C_available_universe.py", "--mode", "o", "--bgn", fix_bgn_date, "--stp", md_stp_date])
print(sep)

sp.run(["python", "01_market_index.py"])
print(sep)

sp.run(["python", "02_test_return.py", "--mode", "o", "--bgn", md_bgn_date, "--stp", md_stp_date])
print(sep)

sp.run(["python", "02_test_return_neutral.py", "--mode", "o", "--bgn", md_bgn_date, "--stp", md_stp_date])
print(sep)
