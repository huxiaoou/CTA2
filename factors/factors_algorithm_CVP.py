...
def factors_algorithm_CVP(
    ...
    base_date = calendar.get_next_date(iter_dates[0], -cvp_window + 1)
...
def cal_factors_exposure_cvp_mp(
        proc_num: int,
        cvp_windows: list[int],
        **kwargs
):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for p in cvp_windows:
        pool.apply_async(factors_algorithm_CVP, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
