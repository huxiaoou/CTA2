...
def factors_algorithm_SKEW(
    ...
    factor_lbl = "SKEW{:03d}".format(skew_window)
    ...
    base_date = calendar.get_next_date(iter_dates[0], -skew_window + 1)
    ...
    for instrument in concerned_instruments_universe:
        ...
        major_return_df[factor_lbl] = -major_return_df["major_return"].rolling(window=skew_window).skew()
...
def cal_factors_exposure_skew_mp(
        proc_num: int,
    ...
    for p in skew_windows:
        pool.apply_async(factors_algorithm_SKEW, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
