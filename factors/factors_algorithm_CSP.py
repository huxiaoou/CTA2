...
def factors_algorithm_CSP(
    ...
    factor_lbl = "CSP{:03d}".format(csp_window)
    ...
    base_date = calendar.get_next_date(iter_dates[0], -csp_window - sgm_window + 1)
    ...
    for instrument in concerned_instruments_universe:
        ...
        cal_rolling_corr(t_major_return_df=major_return_df, t_x=x, t_y=y, t_rolling_window=csp_window, t_corr_lbl=factor_lbl)
...
def cal_factors_exposure_csp_mp(
        proc_num: int,
        csp_windows: list[int],
        **kwargs
):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for p in csp_windows:
        pool.apply_async(factors_algorithm_CSP, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
