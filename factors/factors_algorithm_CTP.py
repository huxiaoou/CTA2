...
def factors_algorithm_CTP(
    ...
    base_date = calendar.get_next_date(iter_dates[0], -ctp_window)
...
def cal_factors_exposure_ctp_mp(
        proc_num: int,
        ctp_windows: list[int],
        **kwargs
):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for p in ctp_windows:
        pool.apply_async(factors_algorithm_CTP, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
