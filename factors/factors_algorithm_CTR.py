...
def factors_algorithm_CTR(
    ...
    base_date = calendar.get_next_date(iter_dates[0], -ctr_window)
...
def cal_factors_exposure_ctr_mp(
        proc_num: int,
        ctr_windows: list[int],
        **kwargs
):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for p in ctr_windows:
        pool.apply_async(factors_algorithm_CTR, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
