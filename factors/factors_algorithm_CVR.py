...
def factors_algorithm_CVR(
    ...
    base_date = calendar.get_next_date(iter_dates[0], -cvr_window + 1)
...
def cal_factors_exposure_cvr_mp(
        proc_num: int,
        cvr_windows: list[int],
        **kwargs
):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for p in cvr_windows:
        pool.apply_async(factors_algorithm_CVR, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
