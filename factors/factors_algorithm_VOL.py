...
def factors_algorithm_VOL(
    ...
    factor_lbl = "VOL{:03d}".format(vol_window)
    __money_scale = 1e4
    ...
    base_date = calendar.get_next_date(iter_dates[0], -vol_window + 1)
    ...
    for instrument in concerned_instruments_universe:
        ...
        major_return_df[factor_lbl] = major_return_df["amount"] / __money_scale
        major_return_df[factor_lbl] = major_return_df[factor_lbl].rolling(window=vol_window).mean()
...
def cal_factors_exposure_vol_mp(
        proc_num: int,
    ...
    for p in vol_windows:
        pool.apply_async(factors_algorithm_VOL, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
