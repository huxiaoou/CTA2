...
def factors_algorithm_SIZE(
    ...
    base_date = calendar.get_next_date(iter_dates[0], -size_window + 1)
...
def cal_factors_exposure_size_mp(
        proc_num: int,
    ...
    for p in size_windows:
        pool.apply_async(factors_algorithm_SIZE, args=(p,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
